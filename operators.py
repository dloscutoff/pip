...
class Operator(tokens.Token):
    def __init__(
        ...
        try:
            ...
            print(
                "Unsupported default value type for operator "
                f"{token} ({function}):",
                type(default),
            )
    ...
    def __repr__(self):
        return (
            f"Operator({self},{self.function},{self.arity},"
...
for precedence, (arity, associativity, *entries) in enumerate(precedenceTable):
    for entry in entries:
        ...
        if arity > 2 and flags & LIST_EACH:
            ...
            msg = (
                "Current implementation cannot handle LIST_EACH or "
